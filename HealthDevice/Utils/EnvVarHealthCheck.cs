...
public class EnvVarHealthCheck : IHealthCheck
{
    ...
    {
        var requiredVars = new[] { "ConnectionStrings__DefaultConnection", "IS_TESTING" };
        var optionalVars = new[] { "SMTP_HOST", "SMTP_PORT", "SMTP_USER", "SMTP_PASSWORD" };

        var missingRequired = requiredVars
            .Where(v => string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable(v)))
            .ToList();

        var missingOptional = optionalVars
            .Where(v => string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable(v)))
            .ToList();

        var allMissing = missingRequired.Concat(missingOptional).ToList();
        var message = allMissing.Any()
